...
def checkLast(lastOccurences, kmerOccurences):
    newKmerOccurences = []
    if lastOccurences == None or kmerOccurences == None:
        return None
    for current in kmerOccurences:
        for last in lastOccurences:
            if last - current == kmerLen:
                newKmerOccurences.append(current)
    #print newKmerOccurences
    return newKmerOccurences

def findKmersSeq(query, sa, firstBWT, BWT, firstToLast, seq):
    occurences = []
    kmers = getKmers(query)
    mismatches = 0
    first = True
    for kmer in kmers:
        kmerOccurences = findSeq(kmer, sa, firstBWT, BWT, firstToLast)
        #print kmerOccurences
        if not first:
            kmerOccurences = checkLast(lastOccurences, kmerOccurences)
        if kmerOccurences == None:
            #print "increase mismatch for kmer:", kmer
            mismatches += 1
        elif mismatches >= maxmismatches:
            return None
        else:
            occurences = kmerOccurences
        lastOccurences = kmerOccurences
        first = False
    return occurences
...
def getKmers(query):
    kmers = []
    for i in range(0, len(query) - kmerLen + 1, kmerLen):
        kmers.append(query[i: i + kmerLen])
    kmers = list(reversed(kmers))
    return kmers

qnames = []
with open( sys.argv[ 1 ] ) as fh:
    rname = fh.next().strip()
    rname = rname.translate(None, '>')
    seq = ""
    for line in fh:
...
with open( sys.argv[ 2 ] ) as fh:
    ...
    for query in fh:
        ...
        if toggle:
            qnames.append(query.strip().upper())
            toggle = False
        else:
            queries.append(query.strip().upper())
            toggle = True
...
#@SQ SN:ref LN:45
print "@SQ\tSN:"+rname+"\tLN:"+str(len(seq))

q = 0
for query in queries:
    #print "query", query
    occurence = findKmersSeq(query, sa, firstBWT, BWT, firstToLast, seq)
    if occurence != None:
        occurences.extend(occurence)

    for i in occurences:
        print qnames[q]+"\t0\t"+rname+"\t"+str(i+1)+"\t255\t"+str(len(query))+"M\t*\t0\t0\t"+str(query)+"\t*"
    del occurences[:]
    q+=1
