...
# =============================================================================================================
# =============================================================================================================
def makeSuffixTree( sequence ):
    mySuffixTree = SuffixTree( len( seq ) )
    for char in seq:
        mySuffixTree.add_char(char)
    return mySuffixTree

SA = []
def makeSuffixArray( sequence ):
    mySuffixTree = SuffixTree( len( seq ) )
    for char in seq:
        mySuffixTree.add_char(char)
    return mySuffixTree.get_suffix_array(SA)

BWT = []
def makeBWT( sequence ):
    mySuffixTree = SuffixTree( len( seq ) )
    for char in seq:
        mySuffixTree.add_char(char)
    sa = mySuffixTree.get_suffix_array(SA)
    BWT = genBWT(SA,sequence)
    return BWT

def makeBwtFirstColumn( sequence ):
    mySuffixTree = SuffixTree( len( seq ) )
    for char in seq:
        mySuffixTree.add_char(char)
    sa = mySuffixTree.makeSuffixArray
    firstColumn = mySuffixTree.genFirstBWT
    return firstColumn

def makeBwtFirstToLast( sequence ):
    mySuffixTree = SuffixTree( len( seq ) )
    for char in seq:
        mySuffixTree.add_char(char)
    sa = mySuffixTree.makeSuffixArray
    firstToLastColumn = mySuffixTree.genFirstToLast
    return firstToLastColumn
